...
from predictor_prod_classes import COCODemo
import glob
import sys
import os
...
for file_name in model_list:
	...
	cfg.MODEL.WEIGHT = "/network/tmp1/bhattdha/coco_dataset_new/model_0175000.pth"
	...
	for i in range(len(division_fact)):
		submission_folder_name = 'val_submission_' + file_name[:-5] + '_' + cfg.MODEL.WEIGHT[-17:-4] + '_' + str(division_fact[i]) + '_' + str(confidence_threshold) + '_' + str(min_image_size) + '_prod_30_classses'
			...
			if not sequence_name.endswith('.zip'):
				...
				for image_file in image_list:
					...
					for index in range(detections.bbox.size()[0]):
						...
						print(detections.get_field('labels')[index].item())
